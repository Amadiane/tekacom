...
import {
  Home, Loader2, Trash2, PlusCircle, Edit2, X, Save,
  RefreshCw, Eye, ChevronLeft, ChevronRight, Image as ImageIcon,
  Check, Calendar, Sparkles, Archive, Type, AlignLeft, Link2
} from "lucide-react";

/**
 * 🎨 HOME POST V4 - INSPIRED BY PARTNER POST
...
const HomePost = () => {
  ...
  const handleEdit = (section) => {
    ...
    setPreview(section.image_url || section.image);
    setShowForm(true);
    setShowList(false);
    scrollToForm();
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Supprimer ${selectedCards.length} section(s) ?`)) return;
    ...
    setSuccessMessage(`✨ ${selectedCards.length} section(s) supprimée(s) !`);
    setSelectedCards([]);
    await fetchSections();
  };
  ...
  const filteredSections = sections.filter(section => {
    if (filterStatus === 'active') return section.is_active;
    if (filterStatus === 'inactive') return !section.is_active;
    return true;
  });
      ...
      <div className="relative max-w-[1800px] mx-auto px-6 py-8">
        ...
        <div className="mb-8 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">Sections Home</h1>
              <p className="text-xs text-gray-500">{filteredSections.length} section{filteredSections.length > 1 ? 's' : ''}</p>
            </div>
          </div>
              ...
              <form onSubmit={handleSubmit} className="p-8">
                ...
                <div className="mb-6">
                  ...
                  {preview ? (
                    <div className="relative group rounded-xl overflow-hidden">
                      <img src={preview} alt="Preview" className="w-full h-48 object-cover bg-white/5" />
                      <button
                        type="button"
                        onClick={() => {
                          setPreview(null);
                ...
                <div className="mb-8">
                  <label className="flex items-center gap-3 p-4 bg-[#41124f]/40 border border-[#a34ee5]/30 rounded-xl cursor-pointer hover:border-[#a34ee5]/50 transition-all">
                    ...
                    />
                    <span className="font-bold text-white">Section active</span>
                    {formData.is_active && (
            ...
            {loading ? (
              ...
              <div className="text-center py-32">
                <div className="w-20 h-20 bg-[#41124f]/30 rounded-3xl flex items-center justify-center mx-auto mb-6">
                  <Home className="w-10 h-10 text-[#a34ee5]" />
                </div>
                <h3 className="text-2xl font-bold text-white mb-2">Aucune section</h3>
                <p className="text-gray-500">Créez votre première section</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {currentItems.map((section) => {
                    const isSelected = selectedCards.includes(section.id);
                    ...
                    return (
                      <div key={section.id}>
                        <div
                          ...
                          onClick={() => toggleCardSelection(section.id)}
                        >
                          <div className="relative h-64 bg-[#0a0a0a] overflow-hidden">
                            {section.image_url || section.image ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center">
                                <Home className="w-16 h-16 text-gray-700" />
                              </div>
                            )}
                            ...
                            <div className="absolute top-3 right-3 flex gap-2">
                              ...
                              {section.order !== undefined && (
                                <span className="px-2 py-1 bg-[#a34ee5]/90 backdrop-blur-sm text-white rounded-lg text-xs font-bold">
                                  #{section.order}
                                </span>
                              )}
                            </div>
                          ...
                          <div className="p-4">
                            ...
                            {section.subtitle && (
                              <p className="text-[#fec603] text-sm font-medium mb-2">
                                {section.subtitle}
                              </p>
                            )}
                            {section.description && (
                            ...
                            {section.cta_text && (
                              <div className="inline-flex items-center gap-1 px-3 py-1 bg-[#a34ee5]/10 border border-[#a34ee5]/30 rounded-full text-[#a34ee5] text-xs">
        ...
        >
          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              {(selectedSection.image_url || selectedSection.image) && (
                <div className="rounded-2xl overflow-hidden border border-[#a34ee5]/20">
                  <img
                    src={selectedSection.image_url || selectedSection.image}
              ...
              {selectedSection.subtitle && (
                <div className="p-4 bg-[#fec603]/10 border border-[#fec603]/30 rounded-2xl">
                  <p className="text-[#fec603] font-bold text-lg">{selectedSection.subtitle}</p>
                </div>
              )}

              {selectedSection.description && (
                <div className="p-6 bg-[#41124f]/20 border border-[#a34ee5]/20 rounded-2xl">
                  <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{selectedSection.description}</p>
                </div>
              )}
                ...
                <div className="p-6 bg-[#a34ee5]/10 border border-[#a34ee5]/30 rounded-2xl">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-white font-bold text-lg">{selectedSection.cta_text}</p>
                    </div>
                    {selectedSection.cta_link && (
                      <a
                        href={selectedSection.cta_link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-6 py-3 bg-gradient-to-r from-[#a34ee5] to-[#7828a8] text-white rounded-xl font-bold hover:scale-105 transition-all flex items-center gap-2"
                      >
                        Voir
                        <Link2 className="w-4 h-4" />
                      </a>
